...
ptp2_header_new(void)
{
    ...
    LOG_PRINTLN(LOG_HEADER_PTP2, LOG_DEBUG, ("PTP2 header new 0x%016" PRIxPTR, (uintptr_t) header));
...
ptp2_header_free(header_t *header)
{
    ...
    LOG_PRINTLN(LOG_HEADER_PTP2, LOG_DEBUG, ("PTP2 header free 0x%016" PRIxPTR, (uintptr_t) header));
...
ptp2_header_encode(netif_t *netif, packet_t *packet, raw_packet_t *raw_packet, packet_offset_t offset)
{
    ...
    if (packet->tail->klass->type != HEADER_TYPE_PTP2) {
        LOG_PRINTLN(LOG_HEADER_PTP2, LOG_ERROR, ("%s header encode: next header is not a %s header but a %s header!",
                log_header_type(HEADER_TYPE_PTP2),
                log_header_type(HEADER_TYPE_PTP2),

...
static header_class_t           klass = {
    .type               = PACKET_TYPE_PTP2_SIGNALING,
...
ptp2_signaling_header_new(void)
{
    ...
    LOG_PRINTLN(LOG_HEADER_PTP2_SIGNALING, LOG_DEBUG, ("PTP2 signaling header new 0x%016" PRIxPTR, (unsigned long) header));
...
ptp2_signaling_header_free(header_t *header)
{
    ...
    LOG_PRINTLN(LOG_HEADER_PTP2_SIGNALING, LOG_DEBUG, ("PTP2 signaling header free 0x%016" PRIxPTR, (unsigned long) header));
...
ptp2_signaling_header_encode(netif_t *netif, packet_t *packet, raw_packet_t *raw_packet, packet_offset_t offset)
{
    ...
    if (packet->tail->klass->type != PACKET_TYPE_PTP2_SIGNALING) {
        return 0;
    }
